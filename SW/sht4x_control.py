...
class SHT45:
    ...
    def measure(self, precision='high'):
        ...
        if precision not in cmd_map:
            raise ValueError(f"precision must be 'high', 'medium', or 'low'")
...
Examples:
  ...
  %(prog)s read -f 10000                 # Use 10kHz I2C speed
    ...
    parser.add_argument('-a', '--addr', type=lambda x: int(x, 0), default=0x44,
                       help='I2C address (default: 0x44)')
    parser.add_argument('-f', '--frequency', type=int, default=10000,
                       help='I2C frequency in Hz (default: 10000)')
    
    subparsers = parser.add_subparsers(dest='command', help='Command to execute')
    
    # Scan command
    parser_scan = subparsers.add_parser('scan', help='Scan I2C bus for devices')
    
    # Info command
    parser_info = subparsers.add_parser('info', help='Display device serial number')
    ...
    parser_read.add_argument('-p', '--precision', choices=['high', 'medium', 'low'],
                            default='high', help='Measurement precision (default: high)')

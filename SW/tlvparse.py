...
MESHCOP_TLV_TYPE_NAME = {value: name for name, value in MESHCOP_TLV_TYPE.items()}
...
def main():
    ...
    while pos < len(data):
        ...
        pos += _len
        name = MESHCOP_TLV_TYPE_NAME[tag]
        if tag == 3:
            print(f"t: {tag:2} ({name}), l: {_len}, v: {val}")
        else:
            print(f"t: {tag:2} ({name}), l: {_len}, v: 0x{val.hex()}")

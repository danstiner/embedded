...
def load_prior_identity(sn: str):
    """If serial `sn` was provisioned before, return its stored identity as a dict so
    re-provisioning reproduces a byte-identical factory-data partition — not just the same
    QR/pairing code (discriminator/passcode/salt) but also the same rotating device ID and
    manufacturing date, which the generator would otherwise regenerate every run. None if no
    prior record."""
    ...
    rd_uid = d.get("rd_uid")
    if rd_uid and rd_uid.startswith("hex:"):  # --rd_uid wants the bare hex string
        rd_uid = rd_uid[len("hex:"):]
    return {
        "discriminator": int(d["discriminator"]),
        "passcode": int(d["passcode"]),
        "salt": salt,
        "rd_uid": rd_uid,
        "date": d["date"],
    }
...
def main():
    ...
    ap.add_argument("--prefix", default="SN",
                    help="Serial-number prefix (default: SN)")
    ap.add_argument("--sn", "--serial-number", dest="sn",
                    help="Reuse this device serial (e.g. SN-0001) instead of minting the next "
    ...
    if args.sn:
        ...
        if prior is None:
            raise SystemExit(
                f"--sn {sn}: no prior record at {OUTPUT_DIR / f'factory_data_{sn}.json'}.\n"
                "Refusing to mint a new identity for an explicit serial. Omit --sn to "
                "auto-assign the next serial.")
    else:
        sn = next_serial(args.prefix)
        prior = None

    if prior:
        disc, passcode, salt = prior["discriminator"], prior["passcode"], prior["salt"]
        rd_uid, date = prior["rd_uid"], prior["date"]
        print(f"Reusing existing identity for {sn} (discriminator={disc} passcode={passcode}) "
              "— byte-identical re-flash")
    else:
        ...
        rd_uid = None                      # generated by the script below
    ...
    # Rotating device ID: replay the stored one for a byte-identical re-flash, else let the
    # script mint a fresh random one (--rd_uid takes precedence over --generate_rd_uid).
    if rd_uid:
        cmd += ["--rd_uid", rd_uid]
    else:
        cmd += ["--generate_rd_uid"]
